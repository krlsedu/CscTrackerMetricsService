...
pipeline {
    ...
    stages {
        ...
        stage('Docker image') {
            ...
            steps {
                sh 'docker build -t krlsedu/csctracker-metrics-service:' + env.VERSION_NAME + ' -t krlsedu/csctracker-metrics-service:latest  .'
            }
        }
        stage('Docker Push') {
            agent any
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerHub', passwordVariable: 'dockerHubPassword', usernameVariable: 'dockerHubUser')]) {
                    ...
                    sh 'docker push krlsedu/csctracker-metrics-service:' + env.VERSION_NAME
                    sh 'docker push krlsedu/csctracker-metrics-service'
                }
            }
        }
